  ...
  <div>
    <template>
        ...
        <el-row :span="24">
          <el-col :span="12">
            <el-form :inline="true" :model="searchPage" class="demo-form-inline">
              <el-form-item label="故事查询">
                <el-input v-model="searchPage.searchText" placeholder="请输入故事标题查询"></el-input>
              </el-form-item>
              <el-form-item>
          ...
          <el-col :span="12" align="right" v-if="multipleSelection.length>0">
            <el-button type="success" @click="setStatusAll(1)" v-role-btn="'63'">批量审核通过</el-button>
            <el-button type="primary" @click="setStatusAll(2)" v-role-btn="'63'">批量审核不通过</el-button>
            <el-button type="danger" @click="delStatusAll" v-role-btn="'64'">批量删除</el-button>
          </el-col>
        </el-row>
      </el-card>
      <el-table
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-tag type="success" disable-transitions>{{scope.row.columnName}}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            ...
            >
              ...
              <el-button size="mini" slot="reference" v-role-btn="'63'">状态修改</el-button>
            </el-popover>
            <el-button size="mini" type="danger" @click="handleDelete(scope.$index, scope.row.id)" v-role-btn="'64'">
...
export default {
  ...
  methods: {
    ...
    getList(data) {
      let _this = this;
      this.$request.fetchGetStoryList(data).then(function (res) {
        if (res.data.code === 200) {
    ...
    // 获取栏目列表
    getColumn(data) {
      this.$request.fetchColumnList(data).then((res) => {
        if (res.data.code === 200) {
  ...
  mounted() {
    this.getList(this.searchPage); // 获取故事列表
    this.getColumn(this.columnPage); // 获取栏目列表
  },
  watch: {
